...
export const TASK_PRIORITY_OPTIONS: Record<NonNullable<Task["priority"]>, BadgeOption> = {
  high:   { label: "Haute",   icon: ArrowUp,   className: "bg-red-50 text-red-700 dark:bg-red-950 dark:text-red-300" },
  normal: { label: "Normale", icon: Minus,      className: "bg-amber-50 text-amber-700 dark:bg-amber-950 dark:text-amber-300" },
...
export function TaskPriorityBadge({ priority, className }: { priority: Task["priority"]; className?: string }) {
  ...
  return <StatusBadge option={TASK_PRIORITY_OPTIONS[priority]} className={className} />;
}
